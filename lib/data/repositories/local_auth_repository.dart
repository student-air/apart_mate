...
class LocalAuthRepository implements IAuthRepository {
  // Fixed IDs so other repositories (society/property) can pre-seed
  // matching demo data for these accounts.
  static const demoGoogleUserId = 'demo_google_user';
  static const demoAppleUserId = 'demo_apple_user';
  ...
  final List<_Account> _accounts = [
    ...
    // Pre-seeded so Google/Apple sign-in always lands on a fully
    // populated dashboard demo instead of an empty new-user flow.
    _Account(
      user: const UserModel(
        id: demoGoogleUserId,
        fullName: 'Ali Khan',
        email: 'ali.khan.demo@gmail.com',
        phone: '03001234567',
        role: 'owner',
      ),
      username: 'ali.khan.demo@gmail.com',
      password: '',
    ),
    _Account(
      user: const UserModel(
        id: demoAppleUserId,
        fullName: 'Ali Khan',
        email: 'ali.khan.demo@icloud.com',
        phone: '03001234567',
        role: 'owner',
      ),
      username: 'ali.khan.demo@icloud.com',
      password: '',
    ),
  ];
  ...
  Future<AuthResult> loginWithGoogle() async {
    await _simulateLatency();
    final account = _accounts.firstWhere((a) => a.user.id == demoGoogleUserId);
    _currentUser = account.user;
    // isNewUser: false -> AuthController routes straight to dashboard.
    return AuthResult(user: account.user, isNewUser: false);
  }

  @override
  Future<AuthResult> loginWithApple() async {
    await _simulateLatency();
    final account = _accounts.firstWhere((a) => a.user.id == demoAppleUserId);
    _currentUser = account.user;
    return AuthResult(user: account.user, isNewUser: false);
  }
  ...
  Future<UserModel> updateUserRole(String role) async {
    await _simulateLatency();
    final user = _currentUser;
    if (user == null) throw Exception('No signed-in user to update');
