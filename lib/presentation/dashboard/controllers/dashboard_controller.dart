...
class DashboardController extends GetxController {
  ...
  /// True when current user is a joined property manager (temporary owner).
  final isActingManager = false.obs;
  ManagerModel? _managerRecord;

  String get userName => _authRepository.currentUser?.fullName ?? '';
  String get userRole => _authRepository.currentUser?.role ?? '';

  String get roleLabel {
    if (isActingManager.value) return 'Manager';
  ...
  Future<void> loadDashboard() async {
    ...
    isLoading.value = true;
    try {
      // ── Manager (temporary owner)? ────────────────────────
      isActingManager.value = false;
      _managerRecord = null;
      if (Get.isRegistered<IManagerRepository>()) {
        try {
          final mgr = await Get.find<IManagerRepository>()
              .getManagerByUserId(user.id);
          if (mgr != null && mgr.status.toLowerCase() == 'joined') {
            isActingManager.value = true;
            _managerRecord = mgr;
          }
        } catch (_) {}
      }
      ...
      if (isActingManager.value && _managerRecord != null) {
        // Assigned units only
        fetchedProperties = [];
        for (final id in _managerRecord!.propertyIds) {
          if (id.isEmpty) continue;
          final p = await _propertyRepository.getPropertyById(id);
          if (p != null) fetchedProperties.add(p);
        }
      } else {
        fetchedProperties =
            await _propertyRepository.getPropertiesForUser(user.id);
      ...
      // Owner may have joined society without property yet
      if (!isActingManager.value) {
        final joinedSocietyId =
            await _societyRepository.getSocietyIdForUser(user.id);
        if (joinedSocietyId != null && joinedSocietyId.isNotEmpty) {
          societyIds.add(joinedSocietyId);
        }
      }
      ...
      if (selectedSoc == null) {
        if (selectedProp != null) {
          selectedSoc = loadedSocieties.cast<SocietyModel?>().firstWhere(
                (s) => s?.id == selectedProp!.societyId,
                orElse: () =>
                    loadedSocieties.isEmpty ? null : loadedSocieties.first,
              );
        } else if (loadedSocieties.isNotEmpty) {
          selectedSoc = loadedSocieties.first;
        }
    ...
    if (Get.isRegistered<AppSession>()) {
      final session = Get.find<AppSession>();
      // Manager is not a true owner role registration
      if (!isActingManager.value &&
          society.value != null &&
          !session.hasOwnerRole.value) {
        session.registerOwner();
      }
    }
  }
  ...
  Future<void> _loadApprovalAndUpdates(
    ...
    // Manager is already approved to act on assigned units
    if (isActingManager.value) {
      approved = true;
      events.add(
        DashboardUpdateItem(
          title: 'Manager access active',
          postedAt: DateTime.now(),
          type: DashboardUpdateType.verification,
        ),
      );
    } else if (selectedSoc != null) {
      try {
        final requestInfo = await _societyRepository.getJoinRequestInfo(
          userId: userId,
          societyId: selectedSoc.id,
        );
        // Support both enum-style and string status from repo
        final statusStr = requestInfo.status.toString().toLowerCase();
        approved = statusStr.contains('approved') ||
            statusStr.contains('accepted');
        ...
        // If join info fails, treat as approved when properties exist
        approved = allProps.isNotEmpty;
      }
    }
  ...
  Future<void> deleteProperty(PropertyModel property) async {
    // Managers should not delete owner's property registration
    if (isActingManager.value) return;
